...
public class OdontologoJpaController implements Serializable {
    ...
    public void create(Odontologo odontologo) {
        ...
        try {
            ...
            for (Turno listaTurnosTurnoToAttach : odontologo.getListaTurnos()) {
                listaTurnosTurnoToAttach
                        = em.getReference(listaTurnosTurnoToAttach.getClass(),
                        listaTurnosTurnoToAttach.getId());
            ...
            for (Turno listaTurnosTurno : odontologo.getListaTurnos()) {
                Odontologo oldOdontologoOfListaTurnosTurno
                        = listaTurnosTurno.getOdontologo();
                ...
                if (oldOdontologoOfListaTurnosTurno != null) {
                    oldOdontologoOfListaTurnosTurno.getListaTurnos().
                            remove(listaTurnosTurno);
                    oldOdontologoOfListaTurnosTurno
                            = em.merge(oldOdontologoOfListaTurnosTurno);
                }
            }
    ...
    public void edit(Odontologo odontologo) throws NonexistentEntityException,
            Exception {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Odontologo persistentOdontologo
                    = em.find(Odontologo.class, odontologo.getId());
            ...
            for (Turno listaTurnosNewTurnoToAttach : listaTurnosNew) {
                listaTurnosNewTurnoToAttach
                        = em.getReference(listaTurnosNewTurnoToAttach.getClass(),
                        listaTurnosNewTurnoToAttach.getId());
            ...
            for (Turno listaTurnosNewTurno : listaTurnosNew) {
                if (!listaTurnosOld.contains(listaTurnosNewTurno)) {
                    Odontologo oldOdontologoOfListaTurnosNewTurno
                            = listaTurnosNewTurno.getOdontologo();
                    listaTurnosNewTurno.setOdontologo(odontologo);
                    listaTurnosNewTurno = em.merge(listaTurnosNewTurno);
                    if (oldOdontologoOfListaTurnosNewTurno != null &&
                            !oldOdontologoOfListaTurnosNewTurno.equals(odontologo)) {
                        oldOdontologoOfListaTurnosNewTurno.getListaTurnos().
                                remove(listaTurnosNewTurno);
                        oldOdontologoOfListaTurnosNewTurno
                                = em.merge(oldOdontologoOfListaTurnosNewTurno);
                    }
                }
            }
            em.getTransaction().commit();
            ...
            if (msg == null || msg.length() == 0) {
                ...
                if (findOdontologo(id) == null) {
                    throw new NonexistentEntityException("The odontologo with id " +
                            id + " no longer exists.");
                }
            }
            throw ex;
    ...
    public List<Odontologo> findOdontologoEntities(int maxResults,
            int firstResult) {
        return findOdontologoEntities(false, maxResults, firstResult);
    }

    private List<Odontologo> findOdontologoEntities(boolean all, int maxResults,
            int firstResult) {
