...
public class PacienteJpaController implements Serializable {
    ...
    public void create(Paciente paciente) {
        ...
        try {
            ...
            for (Turno turnosTurnoToAttach : paciente.getTurnos()) {
                turnosTurnoToAttach
                        = em.getReference(turnosTurnoToAttach.getClass(),
                        turnosTurnoToAttach.getId());
            ...
            for (Turno turnosTurno : paciente.getTurnos()) {
                ...
                if (oldPacienteOfTurnosTurno != null) {
                    ...
                    oldPacienteOfTurnosTurno
                            = em.merge(oldPacienteOfTurnosTurno);
                }
            }
    ...
    public void edit(Paciente paciente) throws NonexistentEntityException,
            Exception {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Paciente persistentPaciente
                    = em.find(Paciente.class, paciente.getId());
            ...
            for (Turno turnosNewTurnoToAttach : turnosNew) {
                turnosNewTurnoToAttach
                        = em.getReference(turnosNewTurnoToAttach.getClass(),
                        turnosNewTurnoToAttach.getId());
            ...
            for (Turno turnosNewTurno : turnosNew) {
                if (!turnosOld.contains(turnosNewTurno)) {
                    Paciente oldPacienteOfTurnosNewTurno
                            = turnosNewTurno.getPaciente();
                    turnosNewTurno.setPaciente(paciente);
                    turnosNewTurno = em.merge(turnosNewTurno);
                    if (oldPacienteOfTurnosNewTurno != null &&
                            !oldPacienteOfTurnosNewTurno.equals(paciente)) {
                        oldPacienteOfTurnosNewTurno.getTurnos().
                                remove(turnosNewTurno);
                        oldPacienteOfTurnosNewTurno
                                = em.merge(oldPacienteOfTurnosNewTurno);
                    }
                }
            }
            em.getTransaction().commit();
            ...
            if (msg == null || msg.length() == 0) {
                ...
                if (findPaciente(id) == null) {
                    throw new NonexistentEntityException("The paciente with id " +
                            id + " no longer exists.");
                }
            }
            throw ex;
    ...
    public void destroy(int id) throws NonexistentEntityException {
        ...
        try {
            ...
            try {
                ...
                throw new NonexistentEntityException("The paciente with id " + id +
    ...
    private List<Paciente> findPacienteEntities(boolean all, int maxResults,
            int firstResult) {
